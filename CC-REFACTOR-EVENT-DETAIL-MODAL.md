# CC-REFACTOR-EVENT-DETAIL-MODAL.md
...
- This is a FULL FILE REPLACEMENT. Back up the original first: `cp src/pages/schedule/EventDetailModal.jsx src/pages/schedule/EventDetailModal.BACKUP.jsx`

---

## OVERVIEW

The EventDetailModal is 1200 lines with 8 tabs, broken icon rendering, game prep workflows that don't belong, and a roster card gallery that duplicates the team page. This refactor strips it down to what belongs:

**Structure:**
- **Header** (always visible, no scroll): Event type color bar, title, team + team color, date/time, venue, opponent (if game), series badge
- **Two tabs:**
  - **Overview:** RSVP summary strip, coaches list, volunteer slots (if game). Quick glance, no scroll needed.
  - **Attendance:** Full player list with photo, name, number, position, RSVP status + Yes/No/Maybe buttons. This is the tab that scrolls.
- **Footer** (always visible): Delete, Close, Edit Event buttons

**What's removed:**
- Game Prep tab (belongs in Game Day Command Center, separate workflow)
- Lineup Builder launch (same)
- Game Completion Modal launch (same)
- Roster card gallery (belongs on team page)
- Photo toggle button
- Duplicate coaches rendering (was in Details AND Coaches tab)

**What's preserved:**
- All data loading (roster, RSVPs, volunteers, coaches)
- RSVP update functionality
- Volunteer assignment functionality
- Edit form (inline editing)
- Series edit/delete menus
- Player card expanded modal (click player name to see detail)
- Coach detail modal

**Files touched:**
- `src/pages/schedule/EventDetailModal.jsx` (full replacement)

---

## PHASE 1 — Replace EventDetailModal

### File: `src/pages/schedule/EventDetailModal.jsx`

**Step 1:** Back up the original:
```bash
cp src/pages/schedule/EventDetailModal.jsx src/pages/schedule/EventDetailModal.BACKUP.jsx
```

**Step 2:** Replace the ENTIRE file contents with the following. This preserves all data loading, RSVP/volunteer logic, edit form, and series menus. Only the render is restructured.

**IMPORTANT:** The new file is long. Copy it EXACTLY. Do not truncate or modify.
...
### What was removed:
- Game Prep tab (lineup builder, game completion, game status)
- Roster card gallery tab
- Coaches as separate tab (now inline in Overview)
- Photo toggle button
- 6 of 8 tabs → 2 tabs
- ~600 lines of code
...
### What was preserved:
- All data loading (roster, RSVPs, volunteers, coaches)
- RSVP update functionality with Yes/No/Maybe buttons
- Volunteer assignment with modal picker
- Edit form (all fields)
- Series edit/delete menus
- Player card expanded on click
- Coach detail modal on click
- Share Game Day card button

### What's new:
- Event type color system in header (navy for games, green for practice, etc.)
- Team color bar (3px) in header
- 2-tab structure: Overview (glanceable) + Attendance (scrollable player list)
- Compact RSVP summary strip
- Coaches as compact rows (not cards)
- Volunteer slots as compact rows with inline assign

### Verification

- Click any event in any calendar view → modal opens
- **Games:** Navy header, opponent + home/away visible, Share button, volunteer section in Overview
- **Practices:** Green-tinted header, no opponent/volunteer section
- **Overview tab:** RSVP counts, coaches list, volunteer slots (games), notes
- **Attendance tab:** Player list with photo, number, name, position, grade, RSVP buttons
- **Edit:** Click Edit → form replaces content. Save/Cancel work. Series menus work.
- **Delete:** Single delete and series delete both work
- **Sub-modals:** Click player → PlayerCardExpanded. Click coach → CoachDetailModal.

### Commit message
```
refactor(schedule): rebuild EventDetailModal — 2 tabs, clean header, remove game prep
```
