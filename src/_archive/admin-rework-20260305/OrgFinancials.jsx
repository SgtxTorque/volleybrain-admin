...
function FinTile({ label, value, sub, color, isDark }) {
  return (
    <div className={`rounded-xl p-3 ${isDark ? 'bg-white/[0.04]' : 'bg-slate-50'}`}>
      ...
      <p className={`text-[10px] font-medium ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{label}</p>
      {sub && <p className={`text-[10px] mt-0.5 ${isDark ? 'text-slate-600' : 'text-slate-300'}`}>{sub}</p>}
    </div>
  )
}
    ...
    <div className={`${cardBg} rounded-2xl shadow-sm p-4`}>
      ...
      {overdueCount > 0 && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-lynx-sky text-white text-sm font-semibold hover:brightness-110 transition"
        >
