...
function WidgetPicker({ onAdd, activeWidgets, role, onClose }) {
  ...
  if (available.length === 0) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
        <div className={`rounded-[18px] p-6 w-full max-w-md mx-4 ${isDark ? 'bg-lynx-charcoal border border-white/10' : 'bg-white border border-lynx-silver'}`}
          ...
          <button onClick={onClose} className="px-4 py-2 rounded-[18px] text-sm font-medium bg-blue-500 text-white hover:bg-blue-600 transition">Close</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className={`rounded-[18px] p-6 w-full max-w-lg mx-4 max-h-[80vh] overflow-y-auto ${isDark ? 'bg-lynx-charcoal border border-white/10' : 'bg-white border border-lynx-silver'}`}
        onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-2">
          {available.map(w => (
            <button key={w.id} onClick={() => { onAdd(w.id); onClose() }}
              className={`w-full flex items-center gap-3 p-3 rounded-[18px] text-left transition ${isDark ? 'hover:bg-white/[0.06] border border-white/[0.06]' : 'hover:bg-lynx-cloud border border-slate-100'}`}>
    ...
    <div ref={containerRef}>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <button onClick={() => setShowPicker(true)}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[18px] text-sm font-medium transition ${isDark ? 'bg-white/[0.06] hover:bg-white/10 text-slate-300 border border-white/[0.06]' : 'bg-white hover:bg-lynx-cloud text-slate-600 border border-lynx-silver'}`}>
          <Plus className="w-4 h-4" /> Add Widget
        </button>

        <button onClick={() => setLocked(!locked)}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[18px] text-sm font-medium transition ${
            locked
      ...
      {activeWidgets.length === 0 ? (
        <div className={`rounded-[18px] border-2 border-dashed p-12 text-center ${isDark ? 'border-white/10' : 'border-lynx-silver'}`}>
          ...
          <button onClick={() => setShowPicker(true)}
            className="px-4 py-2 rounded-[18px] text-sm font-medium bg-blue-500 text-white hover:bg-blue-600 transition">
