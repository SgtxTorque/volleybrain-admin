      ...
      <div className={`grid gap-4 ${teamData.length >= 4 ? 'grid-cols-4' : teamData.length >= 3 ? 'grid-cols-3' : teamData.length >= 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
        {teamData.map((team) => (
          ...
          >
            ...
            />
            <h4 className={`text-r-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{team.name}</h4>
            <p className={`mt-1 text-r-lg ${isDark ? 'text-slate-400' : 'text-lynx-slate'}`}>{team.players} players</p>
            <p className="mt-2 text-r-lg font-semibold" style={{ color: isDark ? '#5eead4' : '#0d9488' }}>{team.record}</p>
          </div>
        ))}
        {teamData.length === 0 && (
