...
// Centered org name, focal health ring, right-aligned needs-attention,
// 2×5 KPI grid at bottom.
// =============================================================================

import { AlertCircle, Users, Shield, DollarSign, FileText, Calendar, UserCheck, CheckCircle2, ChevronRight, ClipboardList, Shirt } from 'lucide-react'
...
function HealthRing({ score, size = 130 }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-[7px] rounded-full bg-[#0B1628] flex flex-col items-center justify-center">
        ...
        <span className="text-[9px] font-bold uppercase tracking-wider mt-1" style={{ color: status.color }}>
          {status.label}
        </span>
      </div>
    </div>
...
function KpiCell({ label, value, icon: Icon }) {
  return (
    <div className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-white/[0.04]">
...
function UrgentItem({ label, count, severity = 'warning', onClick }) {
  ...
  return (
    ...
    >
      <span className="text-sm text-slate-300 flex-1 truncate text-right">{label}</span>
      <div className={`w-2 h-2 rounded-full shrink-0 ${dotColor}`} />
      {count > 0 && (
        <span className="text-xs font-bold text-white bg-white/[0.08] px-2 py-0.5 rounded-md tabular-nums min-w-[28px] text-center">{count}</span>
      )}
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden h-full" style={{ background: 'linear-gradient(135deg, #0B1628 0%, #122240 50%, #0B1628 100%)' }}>
      ...
      <div className="relative z-10 p-4 flex flex-col h-full gap-3">
        ...
        {/* Ring (left) + Needs Attention (right-aligned) */}
              ...
              <div className="space-y-0.5 w-full">
                ...
                {hiddenCount > 0 && (
                  <button
                    ...
                    className="text-xs text-lynx-sky font-bold hover:underline px-1.5 pt-1 ml-auto block"
                  >
        ...
        <div className="grid grid-cols-5 gap-1.5">
          ...
          <KpiCell icon={Shirt} label="Open Spots" value={kpis.openSpots || 0} />
          ...
          <KpiCell icon={ClipboardList} label="Pending" value={kpis.pendingReg || 0} />
        </div>
      </div>
    </div>
  )
