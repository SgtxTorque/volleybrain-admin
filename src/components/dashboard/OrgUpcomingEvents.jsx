...
function EventTypeChip({ type }) {
  ...
  return (
    <span className={`px-1.5 py-0.5 rounded-md text-r-base font-bold uppercase ${cfg.bg} ${cfg.text}`}>
      {cfg.label}
    </span>
  )
}
    ...
    <div className={`${cardBg} rounded-2xl shadow-sm p-4`}>
      ...
      {events.length === 0 ? (
        <div className="text-center py-4">
          <p className={`text-r-lg ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>No upcoming events</p>
          <button
            onClick={() => onNavigate?.('schedule')}
            className="text-r-base text-lynx-sky font-semibold mt-1"
          >
            Create Event
          </button>
        </div>
          ...
          {events.slice(0, 5).map(event => (
            ...
            >
              <div className={`text-center min-w-[44px] shrink-0 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                <p className="text-r-base font-bold">{formatEventDate(event.event_date).split(', ')[0]}</p>
                <p className="text-r-lg">{formatEventDate(event.event_date).split(', ')[1]}</p>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <p className={`text-r-lg font-semibold truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                    {event.opponent_name
                ...
                <div className="flex items-center gap-2 text-r-lg">
                  {event.teams?.name && (
