    ...
    <div className={`${cardBg} rounded-2xl shadow-sm p-4`}>
      ...
      {posts.length === 0 ? (
        <div className="text-center py-4">
          <p className={`text-lg ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>No recent posts</p>
        </div>
      ) : (
          ...
          {posts.map(post => (
            <div key={post.id} className={`p-2.5 rounded-lg ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}>
              <div className="flex items-center gap-2 mb-1.5">
                <div
                  className="w-6 h-6 rounded-full flex items-center justify-center text-base font-bold text-white"
                ...
                <span className={`text-lg ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>
                  {timeAgo(post.created_at)}
                </span>
                {post.teams?.name && (
                  <span
                    className="text-base font-bold px-1.5 py-0.5 rounded-md ml-auto"
                    style={{
              ...
              <p className={`text-base line-clamp-2 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                {post.content}
              </p>
            </div>
          ))}
