      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className={`text-r-lg ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                {season?.name || 'Season'}
              </p>
            </div>
          </div>
          <span className="text-r-4xl font-black tabular-nums" style={{ color: sportColor }}>
            {progressPct}%
          </span>
        </div>
          ...
          {JOURNEY_STEPS.map((step, idx) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center gap-1">
                ...
                <span className={`text-r-base font-medium ${
                  isDone || isActive
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              {teamCount || 0} teams
            </span>
            <span className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          ...
          <button
            ...
            className="flex items-center gap-1 text-r-lg font-semibold transition-colors"
