    ...
    <div className={`flex flex-col gap-5 rounded-2xl p-5 shadow-sm overflow-hidden ${
      ...
      <div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-baseline gap-1.5 ml-auto">
            ...
            <span className={`text-r-lg font-semibold ${isDark ? 'text-slate-400' : 'text-[#0D1B3E]/30'}`}>
              overdue
            </span>
          </div>
        </div>
            ...
            <>
              ...
              <div className="mt-2 flex items-center gap-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-r-lg ${isDark ? 'text-slate-500' : 'text-lynx-slate'}`}>{Math.round(collectedW)}% collected</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-red-500" />
                  <span className={`text-r-lg ${isDark ? 'text-slate-500' : 'text-lynx-slate'}`}>{Math.round(overdueW)}% overdue</span>
                </div>
              </div>
            </>
          )
      ...
      <div className="flex items-stretch gap-0">
        ...
        <div className="flex flex-1 flex-col gap-0 pr-8">
          <span className={`mb-2 text-r-lg font-bold uppercase tracking-[1.2px] ${isDark ? 'text-slate-500' : 'text-lynx-slate'}`}>
            Sources
          </span>
          {[
            ...
            return (
              ...
              >
                ...
                <span className={`text-r-lg ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{source.label}</span>
                <span className={`ml-auto text-r-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="flex flex-1 flex-col gap-0 pl-8">
          ...
          {recentPayments && recentPayments.length > 0 ? (
            recentPayments.slice(0, 5).map((payment, i) => (
              ...
              >
                <span className={`text-r-lg font-semibold truncate ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{payment.name}</span>
                <span className={`text-r-base text-center ${isDark ? 'text-slate-500' : 'text-lynx-slate'}`}>{payment.date}</span>
                <span className={`text-r-base text-center ${isDark ? 'text-slate-400' : 'text-lynx-slate'}`}>{payment.lineItem}</span>
                <span className={`text-r-lg font-semibold text-right ${isDark ? 'text-white' : 'text-slate-900'}`}>{payment.amount}</span>
              </div>
            ))
          ) : (
            <div className={`border-t py-2.5 ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
              <span className={`text-r-lg ${isDark ? 'text-slate-500' : 'text-lynx-slate'}`}>No recent payments</span>
            </div>
          )}
        </div>
      </div>
