...
export function SkeletonCard({ className = '' }) {
  const { isDark } = useTheme()
  return (
    <div className={`p-5 rounded-2xl border ${isDark ? 'bg-lynx-charcoal/60 border-white/[0.06]' : 'bg-white border-lynx-silver'} ${className}`}>
...
export function SkeletonMetricCard({ className = '' }) {
  const { isDark } = useTheme()
  return (
    <div className={`p-4 rounded-2xl border ${isDark ? 'bg-lynx-charcoal/60 border-white/[0.06]' : 'bg-white border-lynx-silver'} ${className}`}>
...
export function SkeletonTable({ rows = 5, cols = 5, className = '' }) {
  const { isDark } = useTheme()
  return (
    <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-lynx-charcoal/60 border-white/[0.06]' : 'bg-white border-lynx-silver'} ${className}`}>
      {/* Header */}
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {Array.from({ length: items }).map((_, i) => (
        <div key={i} className={`flex items-center gap-3 p-3 rounded-2xl border ${isDark ? 'bg-lynx-charcoal/60 border-white/[0.06]' : 'bg-white border-lynx-silver'}`}>
          <SkeletonCircle size="36px" />
          <div className="flex-1 space-y-2">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <SkeletonRect width="120px" height="36px" className="rounded-2xl" />
      </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <SkeletonRect width="100px" height="36px" className="rounded-2xl" />
          <SkeletonRect width="100px" height="36px" className="rounded-2xl" />
        </div>
      </div>
      {/* Status tabs */}
