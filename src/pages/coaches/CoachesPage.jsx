...
export function CoachesPage({ showToast }) {
  ...
  return (
    ...
    >
      ...
      {selectedCoachForDetail && <CoachDetailModal coach={selectedCoachForDetail} onClose={() => setSelectedCoachForDetail(null)} onEdit={() => { setSelectedCoachForDetail(null); setEditingCoach(selectedCoachForDetail); setShowAddModal(true) }} tc={tc} showToast={showToast} orgName={organization?.name} onRefresh={loadCoaches} />}
      {showEmailBlast && (
        <EmailCoachesModal
...
// COACH DETAIL MODAL
// ============================================
export function CoachDetailModal({ coach, onClose, onEdit, tc, showToast, orgName, onRefresh }) {
  ...
  async function handleRegenerateCode() {
    ...
    const { error } = await supabase.from('coaches').update({ invite_code: newCode }).eq('id', coach.id)
    if (error) {
...
function CoachFormModal({ coach, onSave, onClose, showToast, tc }) {
  ...
  function handleSubmit() {
    ...
    onSave({
      ...
      certification_level: clean(form.certification_level),
