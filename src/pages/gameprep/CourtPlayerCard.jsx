...
function CourtPlayerCard({
  ...
  if (!player) {
    return (
      <div
        className="relative aspect-[3/4] rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 transition-all cursor-pointer"
        style={{
      ...
      >
        ...
        <span className="text-base" style={{ color: theme?.textMuted || '#64748b' }}>{position?.label}</span>
        {isServing && (
    ...
    >
      ...
      {isServing && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10">
          <div className="px-3 py-1.5 bg-amber-500 rounded-full text-base font-bold text-black flex items-center gap-1">
            <span>🏐</span> SERVE
          </div>
        </div>
      )}
      ...
      >
        <p className="text-amber-400 font-bold text-lg leading-tight">{player.first_name}</p>
        <p className="text-white font-black text-xl leading-tight truncate">{player.last_name?.toUpperCase()}</p>

        {/* Live stats row */}
        {showStats && (
          <div className="flex gap-4 mt-3 pt-3 border-t border-white/10">
            <div className="text-center">
              <p className="text-2xl font-bold text-red-400">{stats.kills || 0}</p>
              <p className="text-xs text-slate-400 uppercase">Kills</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-emerald-400">{stats.aces || 0}</p>
              <p className="text-xs text-slate-400 uppercase">Aces</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-indigo-400">{stats.digs || 0}</p>
              <p className="text-xs text-slate-400 uppercase">Digs</p>
            </div>
          </div>
        )}
      </div>
      ...
      {isLibero && (
        <div className="absolute top-12 left-2">
          <span className="px-3 py-1 bg-pink-500 text-white text-base font-bold rounded">L</span>
        </div>
      )}
    </div>
  )
