...
function StatusChip({ roster_open, playerCount, maxRoster }) {
  if (playerCount >= maxRoster) {
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-500/12 text-red-500">Full</span>
  }
  if (roster_open) {
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/12 text-emerald-500">Open</span>
  }
  return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-500/12 text-amber-500">Closed</span>
}
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className={isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}>
              {['Team', 'Roster', 'Health', 'Type', 'Status', 'Wall', ''].map(h => (
                <th key={h} className={`px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    {playerCount > 0 ? (
                      ...
                      <span className={`text-xs ${isDark ? 'text-slate-600' : 'text-slate-300'}`}>Empty</span>
                    )}
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <button
                      ...
                      className="text-xs font-semibold text-lynx-sky hover:brightness-110 flex items-center gap-1"
                    >
                      ...
                      <>
                        ...
                        <div className={`absolute right-5 top-12 z-20 rounded-lg shadow-lg border py-1 min-w-[140px] ${
                          ...
                          <button
                            ...
                            className={`w-full px-4 py-2 text-left text-sm flex items-center gap-2 text-red-500 ${isDark ? 'hover:bg-white/[0.04]' : 'hover:bg-red-50'}`}
                          >
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7} className={`text-center py-8 text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
