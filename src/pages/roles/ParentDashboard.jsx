    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className={`${isDark ? 'bg-lynx-charcoal border border-white/[0.08] shadow-lg shadow-black/25' : 'bg-white border border-lynx-silver shadow-sm'} rounded-xl w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-xl`} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-center gap-3">
            <Calendar className={`w-5 h-5 ${isDark ? 'text-slate-400' : 'text-slate-400'}`} />
            <div>
          ...
          {(event.opponent || event.opponent_name) && (
            <div className="flex items-center gap-3">
              <Users className={`w-5 h-5 ${isDark ? 'text-slate-400' : 'text-slate-400'}`} />
...
function ParentDashboard({ roleContext, navigateToTeamWall, showToast, onNavigate }) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {openSeasons.map(season => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${isDark ? 'bg-white/[0.06] hover:bg-white/10' : 'bg-brand-off-white hover:bg-slate-100'} rounded-xl p-4 flex items-center gap-4 transition`}
                >
                  <div className="w-14 h-14 rounded-xl bg-[var(--accent-primary)]/20 flex items-center justify-center text-2xl">
                    {season.sports?.icon || '🏐'}
                  </div>
  ...
  // ============================================
  return (
    <div data-tutorial="dashboard-header" className={`flex h-[calc(100vh-4rem)] overflow-hidden ${isDark ? 'bg-lynx-midnight' : 'bg-brand-off-white'}`}>
