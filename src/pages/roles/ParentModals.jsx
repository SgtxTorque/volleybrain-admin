        ...
        <div className={`p-6 border-b ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'}`}>
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: team?.color || '#6366F1' }}>
              {event.event_type === 'practice' ? <span className="text-r-3xl">{primarySport?.icon || '🏐'}</span> :
               event.event_type === 'game' ? <span className="text-r-3xl">{primarySport?.icon || '🏐'}</span> : '📅'}
            </div>
            <div>
              <h2 className={`text-r-3xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{event.title || event.event_type}</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          {(event.location || venue || event.venue_name) && (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {(venue?.address || event.venue_address) && <p className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{event.venue_address || venue?.address}</p>}
              </div>
            </div>
          )}
          ...
          {event.notes && (
            <div className={`${isDark ? 'bg-white/[0.06]' : 'bg-brand-off-white'} rounded-xl p-4`}>
              <p className={`text-r-lg ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{event.notes}</p>
            </div>
          )}
        </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className={`${isDark ? 'bg-lynx-charcoal border border-white/[0.08] shadow-lg shadow-black/25' : 'bg-white border border-lynx-silver shadow-sm'} rounded-xl w-full max-w-lg shadow-2xl`}>
        <div className={`p-5 border-b ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'}`}>
          <h2 className={`text-r-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Make a Payment</h2>
          <div className="flex items-center justify-between mt-2">
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} text-r-lg`}>Total Due</p>
            <p className="text-r-4xl font-bold text-[var(--accent-primary)]">${amount?.toFixed(2) || '0.00'}</p>
          </div>
        </div>
        ...
        <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
          {fees.length > 0 && (
            <div className={`${isDark ? 'bg-white/[0.06]' : 'bg-brand-off-white'} rounded-xl overflow-hidden`}>
              <button onClick={() => setShowFeeBreakdown(!showFeeBreakdown)}
                ...
                <span className="font-medium text-r-lg">Fee Breakdown</span>
                ...
                <div className={`px-3 pb-3 border-t ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'}`}>
                  {Object.entries(feesByPlayer).map(([playerName, playerFees]) => (
                    <div key={playerName} className="mt-3">
                      <p className={`text-r-base font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-wide mb-1`}>{playerName}</p>
                      {playerFees.map((fee, idx) => (
                        <div key={idx} className="flex justify-between text-r-lg py-1">
          ...
          {hasPaymentMethods && (
            <div className={`${isDark ? 'bg-white/[0.06]' : 'bg-brand-off-white'} rounded-xl p-3`}>
              <p className={`text-r-base ${isDark ? 'text-slate-400' : 'text-slate-500'} mb-1`}>Include this note with your payment:</p>
              <div className="flex items-center gap-2">
                <code className={`flex-1 text-r-lg ${isDark ? 'text-white bg-white/10' : 'text-slate-900 bg-slate-200/60'} px-2 py-1 rounded`}>
                  {paymentNote}
                </code>
                <button onClick={() => copyToClipboard(paymentNote, 'Note')}
                  className={`text-r-base px-2 py-1 rounded ${copied === 'Note' ? 'bg-emerald-500/20 text-emerald-600' : 'bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]'}`}>
                  {copied === 'Note' ? '✓' : 'Copy'}
                </button>
              </div>
            </div>
          ...
          {hasPaymentMethods && (
            <div className="space-y-2">
              <p className={`text-r-base font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-wide`}>Payment Methods</p>
              {organization?.payment_venmo && (
                <a href={`https://venmo.com/${organization.payment_venmo.replace('@', '')}?txn=pay&amount=${amount?.toFixed(2) || '0'}&note=${encodeURIComponent(paymentNote)}`}
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#008CFF] flex items-center justify-center text-white font-bold text-r-xl">V</div>
                  <div className="flex-1">
                    <p className="font-semibold text-[#008CFF]">Venmo</p>
                    <p className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>@{organization.payment_venmo.replace('@', '')}</p>
                  </div>
              ...
              {organization?.payment_zelle && (
                <div className="flex items-center gap-3 p-3 bg-[#6D1ED4]/10 rounded-xl">
                  <div className="w-10 h-10 rounded-full bg-[#6D1ED4] flex items-center justify-center text-white font-bold text-r-xl">Z</div>
                  <div className="flex-1">
                    <p className="font-semibold text-[#6D1ED4]">Zelle</p>
                    <p className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{organization.payment_zelle}</p>
                  </div>
                  <button onClick={() => copyToClipboard(organization.payment_zelle, 'Zelle')}
                    className={`text-r-lg px-3 py-1 rounded-lg transition ${copied === 'Zelle' ? 'bg-emerald-500/20 text-emerald-600' : 'bg-[#6D1ED4]/20 text-[#6D1ED4] hover:bg-[#6D1ED4]/30'}`}>
              ...
              {organization?.payment_cashapp && (
                <a href={`https://cash.app/${organization.payment_cashapp.replace('$', '')}/${amount?.toFixed(2) || '0'}`}
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#00D632] flex items-center justify-center text-white font-bold text-r-xl">$</div>
                  <div className="flex-1">
                    <p className="font-semibold text-[#00D632]">Cash App</p>
                    <p className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{organization.payment_cashapp}</p>
                  </div>
          ...
          {organization?.payment_instructions && (
            <div className={`${isDark ? 'bg-white/[0.06]' : 'bg-brand-off-white'} rounded-xl p-3`}>
              <p className={`text-r-base font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-wide mb-2`}>Additional Instructions</p>
              <p className={`text-r-lg ${isDark ? 'text-slate-300' : 'text-slate-600'} whitespace-pre-wrap`}>{organization.payment_instructions}</p>
            </div>
          )}
          ...
          {!hasPaymentMethods && !organization?.payment_instructions && (
            <div className={`${isDark ? 'bg-white/[0.06]' : 'bg-brand-off-white'} rounded-xl p-6 text-center`}>
              ...
              <p className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'} mt-1`}>Contact your league administrator for payment options.</p>
            </div>
          )}
          ...
          {hasPaymentMethods && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 flex items-start gap-2">
              ...
              <p className={`${isDark ? 'text-amber-400' : 'text-amber-700'} text-r-lg`}>
        ...
        <div className={`p-5 border-t ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'} space-y-2`}>
          {amount > 100 && (
            <button onClick={() => showToast?.('Payment plan requests coming soon!', 'info')}
              className={`w-full py-2 rounded-xl text-r-lg transition ${isDark ? 'border border-white/[0.06] text-slate-400 hover:bg-white/[0.06]' : 'border border-lynx-silver text-slate-500 hover:bg-brand-off-white'}`}>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className={`${isDark ? 'bg-lynx-charcoal border border-white/[0.08] shadow-lg shadow-black/25' : 'bg-white border border-lynx-silver shadow-sm'} rounded-xl w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-xl`}>
        <div className={`p-6 border-b ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'}`}>
          <h2 className={`text-r-3xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>Add Another Child</h2>
          <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} text-r-lg mt-1`}>Select a season to register a sibling</p>
        </div>
            ...
            <>
              {templateChild && (
                <div className={`${isDark ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-emerald-50 border border-emerald-200'} rounded-xl p-4 mb-4`}>
                  <p className={`${isDark ? 'text-emerald-400' : 'text-emerald-700'} text-r-lg`}>
              ...
              {openSeasons.map(season => (
                <a key={season.id} href={getSiblingRegistrationUrl(season)} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="w-14 h-14 rounded-xl bg-[var(--accent-primary)]/20 flex items-center justify-center text-r-3xl">
                    {season.sports?.icon || '🏐'}
                  </div>
                  <div className="flex-1">
                    <p className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{season.name}</p>
                    <p className={`text-r-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{season.organizations?.name}</p>
                  </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className={`${isDark ? 'bg-lynx-charcoal border border-white/[0.08] shadow-lg shadow-black/25' : 'bg-white border border-lynx-silver shadow-sm'} rounded-xl w-full max-w-md shadow-xl`}>
        <div className={`p-6 border-b ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'}`}>
          <h2 className={`text-r-3xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>Re-Register {player.first_name}</h2>
          <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} text-r-lg mt-1`}>for {season.name}</p>
        </div>
      ...
      <div className={`${isDark ? 'bg-lynx-charcoal border border-white/[0.08] shadow-lg shadow-black/25' : 'bg-white border border-lynx-silver shadow-sm'} rounded-xl w-full max-w-lg shadow-xl`} onClick={e => e.stopPropagation()}>
        <div className={`p-6 border-b ${isDark ? 'border-white/[0.06]' : 'border-lynx-silver'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-r-3xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{alert.title}</h2>
