...
### April 13, 2026 (First-Run Flow Gap)
- WEB: Setup wizard now auto-creates programs for each selected sport → Mobile should verify program creation on signup
- WEB: Season creation modal reduced to 2 tabs (Basic Info + Fees) — Registration moved to Lifecycle Tracker Step 2 → Mobile season creation should be updated if it has a registration step
- WEB: RegistrationSetupModal created for Lifecycle Step 2 → Mobile has its own registration flow — no direct parity needed
- WEB: ProgramPage season creation now reads org fee defaults → Mobile should verify same
- WEB: Dashboard CTA cards respect prerequisites → Mobile dashboard should verify same
- WEB: Lifecycle Tracker updated from 7 to 8 steps → Mobile does not have admin tracker — no action
